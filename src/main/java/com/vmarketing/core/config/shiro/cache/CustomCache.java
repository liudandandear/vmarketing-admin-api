...
public class CustomCache<K, V> implements Cache<K, V> {
	...
	private String getKey(Object key) {
		return RedisConstant.PREFIX_SHIRO_CACHE + JwtUtil.getClaim(key.toString(), JwtConstant.ACCOUNT_KEY);
	}
	...
	public Object put(Object key, Object value) throws CacheException {
		...
		// 设置Redis的Shiro缓存
		try {
			redisTemplate.opsForValue().set(this.getKey(key), value, Integer.parseInt(shiroCacheExpireTime),
					TimeUnit.SECONDS);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
